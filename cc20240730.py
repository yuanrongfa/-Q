...
from PyQt5.QtWidgets import QApplication, QMainWindow, QSystemTrayIcon, QMenu, QAction, QWidget, QVBoxLayout, QHBoxLayout, QRadioButton, QTextEdit, QPushButton, QLabel, QMessageBox, QComboBox
...
class SystemTrayApp(QMainWindow):
    def __init__(self):
        ...
        restore_action = QAction("显示界面", self)
    ...
    def initUI(self):
        ...
        # 运行时间选择
        self.time_label = QLabel("请选择运行时间：")
        self.time_combo = QComboBox()
        self.time_combo.addItems(["0.5", "1", "2", "3", "4", "5"])
        self.time_combo.setCurrentIndex(1)  # 默认选择1秒
        self.time_combo.setFixedWidth(50)  # 设置下拉框宽度

        time_layout = QHBoxLayout()
        time_layout.addWidget(self.time_label)
        time_layout.addWidget(self.time_combo)
        time_layout.addStretch()  # 添加弹性空间，确保紧密排列

        button_layout = QHBoxLayout()
        button_layout.addWidget(self.modify_button)
        button_layout.addWidget(self.execute_button)

        layout.addWidget(QLabel("较低风险意见录入参考:"))
    ...
    def execute_action(self):
        # 日期检查
        if datetime.now() > datetime(2025, 7, 31):
            self.show_message("请联系发哥更新！")
            return

        # 获取运行时间
        run_time = float(self.time_combo.currentText())
        ...
        if self.low_risk_radio.isChecked():
            self.low_risk_action(run_time)
        elif self.medium_risk_radio.isChecked():
            self.medium_risk_action(run_time)
        elif self.other_radio.isChecked():
            self.other_action(run_time)
        else:
            self.show_message("请选择一个风险等级")

    def low_risk_action(self, run_time):
        while True:
            # 1. 查找并移动到系统初评.png
            try:
    ...
    def medium_risk_action(self, run_time):
        # 实现中风险操作
        self.show_message("执行中风险操作,可能不会再开发了。")
        # 在这里添加具体的中风险操作代码

    def other_action(self, run_time):
        # 实现其它操作
        self.show_message("执行其它操作，可能不会再开发了。")
        # 在这里添加具体的其它操作代码
